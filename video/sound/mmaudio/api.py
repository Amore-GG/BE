...
class SessionMMAudioRequest(BaseModel):
    """세션 기반 오디오 생성 요청"""
    session_id: str = Field(..., description="세션 ID (공유 폴더 내)")
    video_filename: str = Field(..., description="세션 폴더 내 비디오 파일명")
    output_filename: Optional[str] = Field("mmaudio.mp4", description="출력 파일명 (세션 폴더에 저장)")
    force_rate: Optional[int] = Field(24, description="프레임레이트")
...
@app.post("/session/generate", response_model=MMAudioResponse, tags=["Session"])
async def session_generate_audio(request: SessionMMAudioRequest):
    """
    세션 기반 오디오 생성
    ...
    try:
        ...
        output_filename = request.output_filename or "mmaudio.mp4"
        ...
        return MMAudioResponse(
            success=True,
            output_file=output_filename,
            message=f"세션 '{request.session_id}'에 오디오 생성 영상 저장 완료",
            processing_time=round(processing_time, 2),
